...
export default function NLInputModal({ open, onClose, onSubmit, loading, error }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="w-full max-w-md bg-bg border border-border rounded-xl shadow-lg">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <span className="font-bold text-sm">New workflow</span>
          <button onClick={onClose} className="text-sm text-text-muted hover:text-text transition-colors">
            Cancel
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full bg-bg-subtle border border-border rounded-lg px-3 py-2 text-sm placeholder:text-text-subtle focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent/50 transition-all"
              required
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-bg-subtle border border-border rounded-lg px-3 py-2 text-sm placeholder:text-text-subtle focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent/50 transition-all resize-none"
              required
            />
          </div>
