  ...
  return (
    <div className="w-72 border-l border-border bg-bg-subtle flex flex-col">
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-border">
        ...
        <button onClick={save} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-sand-400 hover:bg-sand-500 rounded-md text-xs text-sand-900 font-medium transition-colors">
        ...
        <div>
          ...
          {(form.tools ?? []).map(t => (
            <div key={t.id} className="flex items-center gap-2 mb-1.5">
              ...
              <button onClick={() => removeTool(t.id)} className="w-6 h-6 flex items-center justify-center rounded-md hover:bg-red-100 text-text-subtle hover:text-red-600 transition-colors flex-shrink-0">
